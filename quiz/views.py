...
def single_quiz(request, quiz_id):
    ...
    quiz.save()
    messages.success(request, f'Hi ,Your {quiz} Open successfully !')
...
def single_question(request, quiz_id, question_id):
    time = timezone.now()  # Time like '23:12:05'%H:%M:%S'
...
def vote(request, quiz_id, question_id):
    ...
    if question_id == (len(quiz.question_set.all())):
        ...
        # messages.success(
        #     request,  f'Hi ,Your {current_question}  complated successfully !')
    ...
    try:
        ...
        if selected_choice == correct_answer:
            # if request.session["num_correct"]:

            print("You are right", selected_choice)
            request.session["num_correct"] += 1
            messages.success(
                request, f'{question_id,selected_choice} answer is right !')
        else:
            print("You are wrong", selected_choice)
            request.session["num_wrong"] += 1
            messages.warning(
                request, f'{question_id,selected_choice} answer is wrong !')
...
def results(request, quiz_id):
    ...
    if num_correct:
        messages.success(request, ' answer is Right !')
    if num_wrong:
    ...
    # except (ZeroDivisionError, UnboundLocalError, NameError):
...
def create_question(request, quiz_id, question_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question = Question(
                quiz=quiz, question_text=question_text, answer=answer, explanation=explanation, question_num=question_id)
            question.save()
            ...
            if question_id == quiz.num_questions:
                return HttpResponseRedirect(reverse('quiz:dashboard'))
            else:
