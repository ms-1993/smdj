...
class CreateQuizForm(forms.Form):
    quiz_name = forms.CharField(max_length=50, label="Quiz Name", widget=forms.TextInput(
        attrs={'class': 'quiz_name_box'}))
    num_questions = forms.IntegerField(label="Number of Questions", widget=forms.NumberInput(
        attrs={'class': 'num_questions_box'}))
    ...
    def clean_num_questions(self):
        ...
        if (isinstance(data, int) == False) or (data < 1):
            raise ValidationError(
                _('Invalid number - must be positive integer'))
...
class CreateQuestionForm(forms.Form):
    question_text = forms.CharField(max_length=10000, label="Question Text", widget=forms.TextInput(
        attrs={'class': 'question_text_box'}))

    choice1_text = forms.CharField(max_length=10000, label="Choice 1", widget=forms.TextInput(
        attrs={'class': 'choice_box'}))
    ...
    choice2_text = forms.CharField(
        max_length=10000, label="Choice 2", widget=forms.TextInput(attrs={'class': 'choice_box'}))
    ...
    choice3_text = forms.CharField(
        max_length=10000, label="Choice 3", widget=forms.TextInput(attrs={'class': 'choice_box'}))
    ...
    choice4_text = forms.CharField(
        max_length=10000, label="Choice 4", widget=forms.TextInput(attrs={'class': 'choice_box'}))
    ...
    explanation = forms.CharField(max_length=10000, label="Explanation Text", widget=forms.TextInput(
        attrs={'class': 'explanation_text_box'}))

    answer = forms.CharField(max_length=10000, label="answer", widget=forms.TextInput(
        attrs={'class': 'answer_text_box'}))
    ...
    def clean(self):
        ...
        answer_list = [choice1_answer, choice2_answer,
                       choice3_answer, choice4_answer]
